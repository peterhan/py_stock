...
class StockNewsWSCN():
    ...
    def content_info_flow(self,type='global'):
    ...
    def content_lives(self,type='a-stock',ts=None):
        ts_cond=''
        if ts!=None:
            ts_cond='&cursor=%s'%ts
    ...
    def finance_macrodatas(self,days_before=1,days_after=5):
        import datetime
        fmt = DATE_FORMAT
    ...
    def market_kline(self, stocks,days=1,secs=None):
        if secs is None:
            secs = days*86400
        else:
            secs = secs
    ...
    def market_rank(self):
        ...
        url = self.apiddc+'/rank?market_type=forexdata&stk_type=index&order_by=none&sort_field=px_change_rate&limit={0}&fields={1}&cursor=0'.format(200,fields)
        jo = self.get_json(url)
        candle = jo['data']['candle']
        fields = jo['data']['fields']
...
if __name__ =='__main__':
    ...
    wscn = StockNewsWSCN()
    stks = ['UMC.NYSE','600438.SS','AMD.NASD','STWD.NYSE','TSLA.NASD','01818.HKEX']
    wscn.content_info_flow()
    wscn.content_lives('us-stock')
    wscn.finance_macrodatas(1,2)
    # wscn.content_lives('a-stock')
    # wscn.content_lives('hk-stock')
    wscn.market_trend(stks)
    # wscn.market_kline(stks,days=30)
    # wscn.market_kline(stks,secs=60)
    # wscn.market_real(stks)
    wscn.market_real()